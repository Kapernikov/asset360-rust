...
1. Goal
-------
- Provide provenance ("blame") for every node in a LinkMLInstance tree after applying a sequence of change stages.
...
- Keep rust-linkml-core neutral and reusable:
  - Core adds stable NodeIds to all LinkMLInstance nodes.
...
- Python ergonomics:
  - LinkMLInstance exposes a read-only `node_id` integer (u64 in Rust, int in Python).
  - Blame is carried separately and attached via a thin view that keeps navigation idiomatic.

3. High-Level Flow
-------------------
1) Start from a base LinkMLInstance (or empty/new).
...
5) Return the final value and blame map; in Python, call `get_blame_info(value, blame_map)` on any LinkMLInstance to retrieve that node's provenance.

4. Core Changes (rust-linkml-core)
----------------------------------
- NodeIds
  - Add `NodeId(u64)` to every LinkMLInstance node (Scalar/List/Mapping/Object).
...
- Patch trace
  ...
  - Change existing `patch/apply` to return `(LinkMLInstance, PatchTrace)`.
  - Implementation:
...
- Python bindings
  - Expose a read-only `node_id` property on LinkMLInstance as an int.
  - Change `py_patch/py_apply` to return `(LinkMLInstance, {"added": [int], "deleted": [int], "updated": [int]})`.
...
--------------------------
- Type parameter on `LinkMLInstance<M>`: strong typing but cascades generics through core and complicates Python; rejected for simplicity.
...
pub fn patch(
    ...
    deltas: &[Delta],
    sv: &SchemaView,
) -> (LinkMLInstance, PatchTrace);

// Optional helpers (read-only)
pub fn node_id_of(v: &LinkMLInstance) -> NodeId;
pub fn iter_nodes<'a>(v: &'a LinkMLInstance) -> impl Iterator<Item = &'a LinkMLInstance>;
// pub fn resolve_path<'a>(v: &'a LinkMLInstance, path: &DeltaPath) -> Option<&'a LinkMLInstance>;
```
...
pub fn apply_deltas(
    ...
    sv: &SchemaView,
) -> (LinkMLInstance, std::collections::HashMap<NodeId, Asset360ChangeMeta>);
